...
<div class="content-page">
    ...
    <div class="content">
        ...
        <div class="page-heading">
           <!-- <h1><i class='fa fa-table'></i>My Application</h1>-->
        </div>
            ...
            <div class="col-md-12">
                <div class="widget pb-4">
                    <div class="widget-header transparent">
                        <h2><strong>My Application</strong>
                        </h2>
...
function listApplications()
{
    ...
    $contact  = DB::query("select * from new_application where broker_id = '".$_SESSION['broker_id']."' order by id DESC");
    if(DB::count() > 0)
    {
        $id = 1;
        foreach($contact as $contacts)
        {
            $application_id = $contacts['id'];
            $application_type  = ucfirst($contacts['customer_type']);
            $application_status = ucfirst($contacts['status']);
            $application_name = ucfirst($contacts['application_name']);
            $step = $contacts['status_step'].".php?application_id=".$application_id;


            $application = DB::queryFirstRow("select application_date,first_name,last_name from new_application_customer where application_id = '$application_id' order by id DESC");
            $date = $application['application_date'];
            $name = ucfirst($application['first_name'])." ".$application['last_name'];
            $url = "";
            ...
            echo"<tr><th scope='row'>$id</th>
                    ...
                    <td>$name</td>
                    <td><b>$application_name</b></td>";

            if($application_status == "Complete")
            {
                echo "<td style='color:#31B411'><b>$application_status</b></td>
               <td><a href='generate_html_pdf.php?application_id=".$application_id."'><button class='btn btn-secondary' type='button'>View Application (PDF)</button></a> </td>";
            }
            else{
                echo "<td style='color:orangered'><b>$application_status</b></td>
            
              <td><a href='application-$step'><button class='btn btn-secondary' type='button'>Go To Application</button></a> </td>";
            }
