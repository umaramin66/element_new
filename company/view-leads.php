...
if(isset($_GET['view-lead'])) {

    $id = $_GET['view-lead'];


    $getBroker           =      DB::queryFirstRow("select * from brokers where id = '$id'");

    $getBrokerFromAdmin  =      DB::queryFirstRow("select * from User where email = '".$getBroker['contact_email']."' and role_type = 'broker'");
}
?>
...
<div class="content-page">
    ...
    <div class="content">
        ...
        <div class="page-heading">
            <h1><i class='fa fa-table'></i>Leads Section</h1>
            ...
            <div class="col-md-12">
                <div class="widget">
                    <div class="widget-header transparent">
                        <h2><strong>Leads Details</strong>
                        ...
                        <div class="table-responsive">
                            <table id="leads_table"  data-sortable class="table table-hover table-striped">
                                ...
                                <tr>
                                    ...
                                    <th> Email</th>
                                    <th data-sortable="false">Assign To</th>
                                </tr>
                                </thead>
                                <tbody>

                                <?php
                                listLeads($getBrokerFromAdmin['id']);
                                ?>
...
<?php
function listLeads($idi)
{

    $contact  = DB::query("select * from leads where broker_id = '$idi'");
    if(DB::count() > 0)
    {
        foreach($contact as $contacts)
        {
            ...
            echo"<tr><th scope='row'>$id</th>
                ...
                <td>$contactNo</td>
                <td>$email </td>";
            echo "<td><form id=\"add-step2\"  method=\"post\" enctype=\"multipart/form-data\">
                                    <div class=\"form-group\"><select class=\"change\">";
            $brokers = DB::query("select * from User where  id <> '$idi' and role_type = 'broker' ");
            echo "<option value='1' disabled selected>Select Broker</option>";
            foreach ( $brokers as $broker)
            {
                $iid = $broker['id'];
                $nname = $broker['name'];
                echo"<option value='$iid#$id'>$nname</option>";
            }
...
<script>
    $(".change").on('change',function()
    {
        ...
        var lead_id = leads[1];
        var broker_id = leads[0];

        $.ajax({
            url: "assets/ajax/assign_leads.php",
            type: 'POST',
            data: {broker_id:broker_id,lead_id:lead_id},
            dataType:'text',
            success: function (data) {

                swal({
                    title: "Status of Lead Successfully Changed",
